...
class TurkishCheckersNNetResNet():
    def __init__(self, game, args):
        ...
        conv1 = Conv2D(args.num_channels, kernel_size=3,
        ...
        """ self.model.compile(
            loss=[self.loss_pi, self.loss_v], optimizer=Adam(args.lr)) """

    def calculate_loss(self):
        self.target_pis = tf.compat.v1.placeholder(
            tf.float32, shape=[None, self.action_size])
        self.target_vs = tf.compat.v1.placeholder(tf.float32, shape=[None])
        self.loss_pi = tf.compat.v1.losses.softmax_cross_entropy(
            self.target_pis, self.pi)
        self.loss_v = tf.compat.v1.losses.mean_squared_error(
            self.target_vs, tf.reshape(self.v, shape=[-1, ]))
        self.total_loss = self.loss_pi + self.loss_v
        update_ops = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            self.train_step = tf.compat.v1.train.AdamOptimizer(
                self.args.lr).minimize(self.total_loss)
