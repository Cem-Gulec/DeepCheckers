...
class TurkishCheckersNNet():
    def __init__(self, game, args):
        ...
        with self.graph.as_default():
            ...
            h_conv8 = Relu(BatchNormalization(self.conv2d(h_conv7, args.num_channels, 'valid'), axis=3, training=self.isTraining))
            h_conv9 = Relu(BatchNormalization(self.conv2d(h_conv8, args.num_channels, 'valid'), axis=3, training=self.isTraining))
            h_conv9_flat = tf.reshape(h_conv9, [-1, args.num_channels*(self.board_x-4)*(self.board_y-4)])
            s_fc1 = Dropout(Relu(BatchNormalization(Dense(h_conv9_flat, 1024), axis=1, training=self.isTraining)), rate=self.dropout) # batch_size x 1024
