...
class TurkishCheckersNNet():
    def __init__(self, game, args):
        ...
        # Renaming functions
        Relu = tf.nn.relu
        Tanh = tf.nn.tanh ##değişebilir
        BatchNormalization = tf.layers.batch_normalization
        ##başka layer da eklenebilir
        Dropout = tf.layers.dropout
        Dense = tf.layers.dense

        # Neural Net
        self.graph = tf.Graph() ##prosedür- yani tf işlemleri ve operatörleri burada tutuyor
    ...
    def conv2d(self, x, out_channels, padding): ## kaç kere ekleneceği değişebilir
        return tf.layers.conv2d(x, out_channels, kernel_size=[3, 3], padding=padding)

    def calculate_loss(self): ##optimizer ve cross_entropye bakılacak
